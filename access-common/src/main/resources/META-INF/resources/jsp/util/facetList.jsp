...
<c:forEach items="${facetFields}" var="facetField">
	<c:set var="searchFacet" value="${searchState.facets[facetField.name]}" />
	<c:set var="facetLimit">
		<c:if test="${not empty searchFacet && not empty searchFacet.facetLimit}">${searchFacet.facetLimit}</c:if>
	</c:set>
